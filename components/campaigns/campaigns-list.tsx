...
export function CampaignsList() {
  ...
  const deleteCampaign = async (campaign: CampaignRow) => {
    ...
    const confirmed = window.confirm(
      `Excluir a campanha "${campaign.name}"? A cadencia e jobs vinculados tambem serao removidos.`,
    )
    if (!confirmed) return
  ...
  const dispatchNow = async (campaign: CampaignRow) => {
    ...
    if (!res.ok || !json.ok) {
      setError(json.error || 'Nao foi possivel disparar.')
    } else {
      setDispatchResult({
        campaignId: campaign.id,
        message: `${json.sent ?? 0} enviado(s)${json.skipped ? `, ${json.skipped} pulado(s)` : ''}${json.failed ? `, ${json.failed} com erro` : ''}`,
      })
    }
    setDispatchingId(null)
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-bold text-[#2C3E50]">Cadencias de prospeccao</h1>
          <p className="mt-2 text-sm text-slate-600">
            Campanhas ficam em rascunho ate voce aprovar templates, horarios e volume. Neste MVP,
            aprovar ainda nao inicia disparos automaticos.
          </p>
      ...
      <section className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        <div className="flex gap-2">
          ...
          <p>
            Status <strong>aprovada</strong> significa que a campanha esta pronta para a engine.
            O envio outbound automatico ainda nao esta ligado nesta fase.
          </p>
        </div>
      </section>
      ...
      {dispatchResult ? (
        <p className="rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm font-medium text-green-700">
          Disparo concluido: {dispatchResult.message}
        </p>
      ) : null}
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        {campaigns.length ? (
                  ...
                  {(campaign.status === 'approved' || campaign.status === 'active') && (
                    <div className="flex flex-col items-end gap-0.5">
                      <button
                        ...
                        title="Aciona a engine so para esta campanha agora, sem esperar o cron"
          ...
          <div className="p-10 text-center">
            ...
            <p className="mt-4 text-sm font-semibold text-slate-900">Nenhuma campanha ainda</p>
            <p className="mt-1 text-sm text-slate-500">
              Crie a primeira campanha para configurar cadencia e templates.
            </p>
          </div>
        )}
      </section>

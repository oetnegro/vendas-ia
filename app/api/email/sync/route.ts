...
export async function POST(request: NextRequest) {
  ...
  try {
    body = (await request.json()) as SyncEmailBody
  } catch {
  ...
  try {
    ...
    return jsonError('Configure SUPABASE_SERVICE_ROLE_KEY no .env.local para sincronizar Gmail.', 500)
  }
  ...
  if (!connection?.access_token_ciphertext || !connection.google_email) {
    return jsonError('Conecte uma conta Google antes de sincronizar.', 400)
  }
  ...
  const syncResult = await syncWorkspaceGmail({
    admin,
  ...
  if (body.autoReply !== false) {
    try {
      aiReplyResult = await processAutomaticEmailReplies({
        admin,
        workspaceId: targetWorkspaceId,
        actorUserId: userId,
        connection: {
      ...
      errors.push({
        threadId: 'ai_engine',
        message: error instanceof Error ? error.message : 'Erro desconhecido na engine de IA.',
      })
    }
  }
  ...
  return NextResponse.json({
    ok: true,
    syncedThreads: syncResult.syncedThreads,
    syncedMessages: syncResult.syncedMessages,
    inboundMessages: syncResult.inboundMessages,
    aiReplies: aiReplyResult,
    errors,
  })
}
